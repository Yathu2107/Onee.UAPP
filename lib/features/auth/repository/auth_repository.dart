...
class AuthRepository {
  ...
  /// Backend returns [FELoggedUserDetailsModel] directly (not Message wrapper).
  Future<ApiResponse<UserDetails>> getLoggedUserDetails() async {
    try {
      final response = await _dio.get(
        '${AppConstants.accountsPath}/get-logged-user-details',
      );
      ...
      // Error envelope (status/code/text) vs raw user payload.
      if (map.containsKey('status') && map['status'] != null) {
        return _parseResponse(
          response,
          (raw) {
            if (raw is Map) {
              return UserDetails.fromJson(Map<String, dynamic>.from(raw));
            }
  ...
  Future<ApiResponse<String>> updateUser({
    required String name,
    required String email,
    required String phoneNumber,
    String? imagePath,
    ...
    try {
      ...
      final response = await _dio.put(
        '${AppConstants.accountsPath}/update-user',
        data: formData,
      );
      return _parseResponse(response, (raw) => raw?.toString() ?? '');
    } on DioException catch (e) {
  ...
  ApiResponse<T> _parseResponse<T>(
    ...
    if (statusCode == 403 || api.code == 'WRONG_APP') {
      throw ApiException(
        message:
            'This number belongs to Worker/Admin. Please use the correct app.',
        code: 'WRONG_APP',
        statusCode: statusCode,
      );
    }
    ...
    if (statusCode == 401 || statusCode == 404) {
      throw ApiException(
        message: api.text.isNotEmpty ? api.text : 'Session expired. Please log in again.',
  ...
  String _friendlyMessage(String code, String text) {
    switch (code) {
      ...
      case 'WRONG_APP':
        return 'This number belongs to Worker/Admin. Please use the correct app.';
      default:
