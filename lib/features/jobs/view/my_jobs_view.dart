  ...
  String _emptyMessage(MyJobsFilter filter) {
    switch (filter) {
      case MyJobsFilter.all:
        return 'No jobs yet.\nDescribe a problem on Home to create one.';
            ...
            Text(
              activeCount > 0
                  ...
                  : totalCount > 0
                      ...
                      : 'Track requests from offer to completion',
              style: TextStyle(
                fontSize: 14,
            ...
            SingleChildScrollView(
              ...
              child: Row(
                children: [
                  _FilterChip(
                    label: 'All',
                    selected: selected == MyJobsFilter.all,
                    onTap: () => onFilter(MyJobsFilter.all),
                  ),
                  ...
                  _FilterChip(
                    label: 'Cancelled',
                    selected: selected == MyJobsFilter.cancelled,
                    onTap: () => onFilter(MyJobsFilter.cancelled),
                  ),
                ],
              ),
            ),
...
class _FilterChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Material(
      color: selected ? AppColors.nearBlack : AppColors.white.withValues(alpha: 0.72),
...
class _JobRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final date = job.createdOn != null
        ...
        : null;
    final meta = [
      if (job.categoryName?.trim().isNotEmpty == true) job.categoryName!.trim(),
      if (job.workerName?.trim().isNotEmpty == true) job.workerName!.trim(),
      if (date != null) date,
    ].join(' · ');
