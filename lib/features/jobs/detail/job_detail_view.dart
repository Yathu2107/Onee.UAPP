...
import 'job_requesting_view.dart';
...
class JobDetailView extends GetView<JobDetailController> {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      if (JobStatuses.isOffering(job.status)) {
        return JobRequestingView(controller: controller, job: job);
      }
                ...
                child: RefreshIndicator(
                  ...
                  child: CustomScrollView(
                    ...
                    slivers: [
                      ...
                      if (job.amount != null)
                        SliverToBoxAdapter(child: _AmountRow(amount: job.amount!)),
...
class _DetailHero extends StatelessWidget {
  ...
  String get _headline {
    if (JobStatuses.isAccepted(job.status)) return 'Worker accepted';
    if (job.status?.toLowerCase() == 'ongoing') return 'Job in progress';
  ...
  String get _subtitle {
    if (JobStatuses.isAccepted(job.status)) {
      return 'Chat with your worker and track the job from here.';
    }
    if (job.status?.toLowerCase() == 'ongoing') {
      return 'Your worker is on it. Message them anytime.';
    }
    if (JobStatuses.isCompleted(job.status)) {
      return 'Thanks for using Onee.';
    }
...
class _WorkerCard extends StatelessWidget {
  const _WorkerCard({required this.job});

  final JobDetail job;

  @override
  Widget build(BuildContext context) {
    final name = job.workerName?.trim().isNotEmpty == true
        ? job.workerName!.trim()
        : 'Assigned worker';
...
class _DetailActions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final canCancel = JobStatuses.canCancel(status);
    final canRate = JobStatuses.canRate(status, job.hasRating);
    final canComplaint = JobStatuses.canComplaint(status);

    if (!canChat && !canCancel && !canRate && !canComplaint) {
      ...
      child: Column(
        ...
        children: [
          ...
          if (canComplaint) ...[
            ...
            TextButton(
              onPressed: controller.goComplaint,
              child: const Text(
                'Create complaint',
                style: TextStyle(
                  fontWeight: FontWeight.w700,
